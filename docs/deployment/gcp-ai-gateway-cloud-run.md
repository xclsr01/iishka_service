...
curl -X POST https://<ai-gateway-url>/v1/providers/gemini/chat/respond \
  ...
  -d '{"model":"gemini-3.0-flash","messages":[{"role":"user","content":"ping"}]}'
```
...
curl -X POST https://<ai-gateway-url>/v1/providers/gemini/chat/respond \
  ...
  -d '{"model":"gemini-3.0-flash","messages":[{"role":"user","content":"Reply pong"}]}'
```
